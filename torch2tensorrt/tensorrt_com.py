...
if __name__ == "__main__":
    ...
    pthPath = "./weights/yolov5s-face.pt"
    model = torch.load(pthPath, map_location=device)['model']
    model.to(device).eval()
    image_size=320
    trt_engine_path = "./weights/yolov5s-face.trt"
    if 0: # 传统方法，通过onnx转换，这样生成的模型，输入输出为np.ndarray
        onnx_model_path = "./weights/yolov5s-face.onnx"
        Torch_to_ONNX(model, [1,3,320,320], onnx_model_path, device)
        ONNX_to_TensorRT(max_batch_size=1, 
                        fp16_mode=False, 
                        onnx_model_path=onnx_model_path, 
                        trt_engine_path=trt_engine_path)
    else: # nvidia torch2trt tool，这样生成的模型，输入输出和pth模型一致，为torch.Tensor
